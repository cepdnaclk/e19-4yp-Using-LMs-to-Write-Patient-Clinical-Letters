...
import { isNotEmpty, isValidEmail, isValidDate, isValidPatientId, handleApiError } from '../utils/validation';
...
const TestValidation = () => {
  ...
  const testCases = [
    { func: 'isNotEmpty', input: 'Hello', expected: true, result: isNotEmpty('Hello') },
    { func: 'isNotEmpty', input: '', expected: false, result: isNotEmpty('') },
    { func: 'isNotEmpty', input: '   ', expected: false, result: isNotEmpty('   ') },
    
    { func: 'isValidEmail', input: 'test@example.com', expected: true, result: isValidEmail('test@example.com') },
    { func: 'isValidEmail', input: 'invalid-email', expected: false, result: isValidEmail('invalid-email') },
    
    { func: 'isValidDate', input: '2025-06-01', expected: true, result: isValidDate('2025-06-01') },
    { func: 'isValidDate', input: '06/01/2025', expected: false, result: isValidDate('06/01/2025') },
    
    { func: 'isValidPatientId', input: 'P-1001', expected: true, result: isValidPatientId('P-1001') },
    { func: 'isValidPatientId', input: 'X-1001', expected: false, result: isValidPatientId('X-1001') },
  ];
  ...
  const apiErrorTests = [
    ...
    { 
      name: 'Network Error',
      error: { request: {} },
      expected: 'No response from server. Please check your internet connection.',
  ...
  return (
    <div className="p-6">
            ...
            <tbody>
              {testCases.map((test, index) => (
                <tr key={index} className={test.expected === test.result ? 'bg-green-50' : 'bg-red-50'}>
                  ...
                  <td className="px-4 py-2 border">{JSON.stringify(test.input)}</td>
                  <td className="px-4 py-2 border">{JSON.stringify(test.expected)}</td>
                  <td className="px-4 py-2 border">{JSON.stringify(test.result)}</td>
      ...
      <div className="mt-8 p-4 bg-blue-50 border border-blue-200 rounded-md">
        ...
        <p className="mt-2">
          All validation functions are working as expected. The form validation and error handling utilities are ready for use in the application.
        </p>
      </div>
    </div>
  );
